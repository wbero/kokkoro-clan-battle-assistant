...
class ClockRecognizer(
    ...
    decisionScorer: ((BinaryImage, Int) -> Double)? = null,
    nccScorer: ((PixelImage, PixelImage) -> Double)? = null
) {
    private val digitMatcher = DigitMatcher(
        templates = templates,
    ...
    private fun diagnosticMatch(slot: DigitSlot, image: PixelImage, range: IntRange): DiagnosticMatch {
        val match = digitMatcher.match(image, range, includeDiagnostics = true)
        val decisionScores = requireNotNull(match.decisionScores)
        val rawRanking = decisionScores.entries.sortedByDescending(Map.Entry<Int, Double>::value)
    ...
    private fun buildCandidates(
        minuteMatch: DigitMatchResult,
        tensMatch: DigitMatchResult,
        onesMatch: DigitMatchResult
    ): List<ClockCandidate> {
    ...
    private fun bestMatch(image: PixelImage, range: IntRange): DigitMatchResult =
        digitMatcher.match(image, range, includeDiagnostics = false)
