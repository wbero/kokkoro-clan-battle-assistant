...
enum class HorizontalAnchor { CENTER, RIGHT, TOP_HUD, RIGHT_CONTROL, LOADING }
...
object GameCoordinateCalibration {
    ...
    @Volatile private var loadingDeltaX = 0f
    ...
    fun horizontalDelta(anchor: HorizontalAnchor): Float = when (anchor) {
        ...
        HorizontalAnchor.LOADING -> loadingDeltaX
    }
    ...
    fun update(anchor: HorizontalAnchor, deltaX: Float) {
        when (anchor) {
            ...
            HorizontalAnchor.LOADING -> loadingDeltaX = deltaX
        }
    }
}
...
object GameCoordinateMapper {
    ...
    fun mapX(
        ...
        val baseOffset = when (anchor) {
            ...
            // AUTO/global SET are placed inside the game's safe viewport, then
            // independently calibrated because wider layouts may pull the rail
            // toward or away from the safe-area edge.
            HorizontalAnchor.RIGHT_CONTROL -> viewport.spareX / 2f
            // The loading indicator also belongs to the centered game viewport;
            // its animation is calibrated independently from the start button.
            HorizontalAnchor.LOADING -> viewport.spareX / 2f
        }
