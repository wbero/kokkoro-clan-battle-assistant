        ...
        addView(LinearLayout(this@AxisGuideActivity).apply {
            ...
            addView(body(
                "顺序轴：适合固定时间轴。每个动作依次进入队列；点击=角色N 会执行“SET 开启 → 等待该角色 UB → SET 关闭”的完整生命周期。\n\n" +
                    "开关轴：适合条件触发和状态切换。每个节点必须声明五名角色的完整 SET 状态与 AUTO 状态；节点触发后一次计算全部差异，SET 在前、AUTO 在后按点击间隔连续提交，最后确认完整目标状态。"
            ))
            ...
            addView(body(
                "• 使用 UTF-8 文本；空行忽略，# 开头为注释。\n" +
                    ...
                    "• UB后=角色N 只能使用角色1～角色5；卡帧= 支持角色1～角色5或AUTO，均不能使用角色别名。\n" +
                    "• 可在表头填写 角色1UB～角色5UB=游戏内实际技能名；五个要么全部不填，要么全部填写。\n" +
                    "• 角色N=昵称用于显示/顺序轴点击别名，不参与 UB 身份识别；角色NUB 才用于浪花中间技能名识别。\n" +
                    "• 同一节点不能同时写 UB后 和 卡帧。\n" +
            ...
            addView(body(
                "定时：1:20 | ...\n" +
                    ...
                    "角色卡帧：1:10 | 卡帧=角色4 | ...（确认时点菜单角色4，返回后等待角色4 UB并关闭SET）\n" +
                    "AUTO卡帧：1:10 | 卡帧=AUTO | ...（确认时直接点击暂停菜单中的AUTO）\n\n" +
                    "填写角色NUB后，角色 UB 身份优先由浪花中间的技能名识别；TP/头像只作为辅助诊断。\n" +
            ...
            addView(body(
                "• 顺序轴缺少 [轴]。\n" +
                    ...
                    "• 已启用角色NUB识别，但五个角色没有全部填写，或两个角色填写了完全相同的 UB 名称。\n" +
